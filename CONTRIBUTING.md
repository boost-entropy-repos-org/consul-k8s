...
1. The test for `MatchesConsul` will look like:
    ...
    ```
1. The test for `ToConsul` will re-use the same struct as for the previous test cases
   `empty field matches` and `all fields set matches` but renamed to `empty fields` and `every field set`
   and with any `Namespace` set on the Consul struct removed and `CreateIndex`/`ModifyIndex` removed
...
1. `cd hack/crds-to-consul-helm`
1. `go run main.go <path-to-consul-helm>` with the path (relative or absolute) to your consul-helm
   repository on disk. This will copy your CRD there with the required formatting. 
1. Update `templates/controller-mutatingwebhookconfiguration` in consul-helm with the webhook for this resource
...
### Test
1. Build a Docker image for consul-k8s
